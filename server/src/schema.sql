...
-- Notificações in-app (destinadas a um jogador).
CREATE TABLE IF NOT EXISTS notifications (
  id            INTEGER PRIMARY KEY AUTOINCREMENT,
  user_id       INTEGER NOT NULL REFERENCES players(id) ON DELETE CASCADE,
  type          TEXT NOT NULL,   -- friend_request | friend_accept | waitlist_promoted
  actor_id      INTEGER REFERENCES players(id) ON DELETE SET NULL,     -- quem disparou
  tournament_id INTEGER REFERENCES tournaments(id) ON DELETE CASCADE,  -- contexto opcional
  message       TEXT,
